...
def loadDataSet():
    ...
    data_set = [line.split() for line in open("Ar_data.csv", encoding='utf-8').readlines()]
    #    data_set = []
...
def createC1(dataset):
    ...
    # dict={}
    # for item in set1:
    #     dict.update({item:C1.count(item)})
    # dictTest=sorted(dict.items(),key=lambda x:x[1],reverse=True)
    # print(dictTest)
    # return dictTest
    C2 = set()
    for t in C1:
...
def apriori(dataset, minSupport=0.02):
    ...
    L1, supportData = scanDataSet(D, C1)
    # print "L1=", L1, "\n", "outcome: ", supportData
    ...
    while (len(L[k - 2]) > 0):
        ...
        # print 'Ck', Ck
        Lk, supportLk = scanDataSet(D, Ck)  # 计算候选数据集 CK 在数据集 D 中的支持度，并返回支持度大于 minSupport 的频繁项集Lk
...
if __name__ == "__main__":
    ...
    print(scanDataSet(dataset, createC1(dataset),0.002))
    L,returnSupportData = apriori(dataset,minSupport=0.002)
    l = []
    for Lk in L:
        for freq_set in Lk:
